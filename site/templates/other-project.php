...
<main class="main page" role="main">
  ...
  <article class="full <?php if ($page->show_image() == 'true') { echo ' episode '; } else { echo ' default '; } ?>" itemscope itemtype="https://schema.org/CreativeWork">
      ...
      <?php if($page->show_image() == 'true'):  ?>
        <figure>
          <?php if ($page->show_different_image() == "yes" && $page->page_image()->isNotEmpty()) { ?>
            <img itemprop="image" src="<?= $page->page_image()->toFile()->url(); ?>" class="cover <?= $page->page_image()->toFile()->extension(); ?>" alt="<?php echo $page->title() ?>">
          <?php } else if ($page->cover() != "") { ?>
            <img itemprop="image" src="<?= $page->cover()->toFile()->url(); ?>" class="cover <?= $page->cover()->toFile()->extension(); ?>" alt="<?php echo $page->title() ?>">
          <?php } else { ?>
            <img itemprop="image" src="<?= $image->toFile()->url(); ?>" class="cover <?= $image->toFile()->extension(); ?>" alt="<?php echo $page->title() ?>">
          <?php } ?>
        </figure>
      <?php endif ?>
    <?php endif ?>
    ...
    <div class="article-text" style="margin-top:1em;">
      ...
      <!-- GitHub Repo -->
      <div class="github-repo-link-holder">
        <?php if ($page->github_repo()->isNotEmpty()) { ?>
          <a class="action github" href="<?php echo $page->github_repo() ?>" title="GitHub Repository: <?= $page->title(); ?>"  target="_blank">
