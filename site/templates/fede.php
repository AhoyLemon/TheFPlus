...
?>
  <channel>
    ...
    <?php foreach($site->find('episode')->children()->listed()->sortBy('date', 'desc') as $item): ?>
      <?php 
      ...
      <?php if ($item->episode_file() != "") { ?>
        <?php $itemCount++; ?>
        <item>
          <title><?php echo $item->uid() ?>: <?php echo $item->title()->xml(); ?></title>
          <link><?php echo $item->url(); ?></link>
          <guid><?php echo $item->url(); ?></guid>
          <pubDate><?= $item->date()->toDate('Y-m-d') . 'T' . $item->time('H:i') .':00-6:00'; ?></pubDate>
          ...
          <itunes:author>The F Plus</itunes:author>
          <?php if ($item->featured_site() != "") { ?>
            <itunes:subtitle>reading <?= $item->featured_site() ?></itunes:subtitle>
          <?php } else { ?>
            <itunes:subtitle><?= $item->text()->excerpt(100)->xml(); ?></itunes:subtitle>
          <?php } ?>
